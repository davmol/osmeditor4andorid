...
public class SimpleActionModeCallback extends EasyEditActionModeCallback implements android.view.MenuItem.OnMenuItemClickListener {
    ...
    public enum SimpleAction {
        ...
        /**
        NODE_LOC(R.string.menu_add_loc_node, R.string.menu_add_loc_node, new SimpleActionCallback() {

            @Override
            public void action(final Main main, final EasyEditManager manager, final float x, final float y) {

                Location lastLocation = main.getLastLocation();
                Log.i("MEINELOCATIONE: ", lastLocation.toString());
                de.blau.android.Map map = main.getMap();

                Node node = App.getLogic().performAddNode(main, lastLocation.getLongitude(), lastLocation.getLatitude());
                Log.i("MEINELOCATIONE", node.toString());
                main.startSupportActionMode(new NodeSelectionActionModeCallback(manager, node));
                main.performTagEdit(node, null, false, true, false);
            }
        }),
        ...
        /**
        WAY_LOC(R.string.menu_add_loc_way, R.string.simple_add_way, new SimpleActionCallback() {

            @Override
            public void action(final Main main, final EasyEditManager manager, final float x, final float y) {
                Log.i("MEINELOCATIONE", "action");
                boolean loc = true;
                main.startSupportActionMode(new PathCreationActionModeCallback(manager,loc));
            }
        }),
         */
